...
@router.get("/layout/{layout_id}/pages")
async def get_page_range(
    layout_id: int,
    start_page: int = Query(1, ge=1, description="Starting page number"),
    end_page: Optional[int] = Query(None, description="Ending page number"),
    db: aiosqlite.Connection = Depends(get_async_db)
):
    """
    Get a range of pages for a specific Mushaf layout
    
    Useful for preloading multiple pages or getting page summaries.
    """
    try:
        # Verify layout exists and get total pages
        cursor = await db.execute("""
            SELECT total_pages FROM mushaf_layouts WHERE id = ?
        """, (layout_id,))
        layout_row = await cursor.fetchone()
        
        if not layout_row:
            raise HTTPException(status_code=404, detail=f"Mushaf layout {layout_id} not found")
        
        total_pages = layout_row[0]
        
        # Set end_page if not provided
        if end_page is None:
            end_page = min(start_page + 9, total_pages)  # Default to 10 pages max
        ...
        cursor = await db.execute("""
            ...
            FROM pages 
            WHERE mushaf_layout_id = ? AND page_number BETWEEN ? AND ?
            ORDER BY page_number
        """, (layout_id, start_page, end_page))
        page_rows = await cursor.fetchall()
        
        pages = []
        for page_row in page_rows:
            pages.append({
                "page_number": page_row[0],
                "image_url": page_row[1],
                "layout_id": layout_id
            })
